...
def _compute_kern(X, Y=None, metric='rbf', n_jobs=None, sigma=None):
    """Computes an RBF kernel matrix using the median l2 distance as the bandwidth"""
    X = _check_2d(X)
    Y = _check_2d(Y)
    if sigma is None:
...
class KCD(KSampleTest):
    ...
    **kwargs
    ...
    def __init__(self, compute_distance=None, reg=0.001, workers=None, **kwargs):
    ...
    def statistic(self, X, Y, z):
        ...
        """
        K, sigma_x = _compute_kern(X, n_jobs=self.workers)
        L, sigma_y = _compute_kern(Y, n_jobs=self.workers)
    ...
    def _statistic(self, K, L, z):
        """Helper function for efficient permutation"""
        # Compute W matrices from z
    ...
    def witness(self, X, Y, z, X_wit, Y_wit):
        ...
        # Witness distances
        K, _ = _compute_kern(X, X_wit, n_jobs=self.workers, sigma=sigma_x)
        L, sigma_y = _compute_kern(Y, Y_wit, n_jobs=self.workers)
        self.sigma_y_ = sigma_y
    ...
    def conditional_dmat(self, X, Y):
        ...
        K, _ = _compute_kern(X, n_jobs=self.workers)
        L, _ = _compute_kern(Y, n_jobs=self.workers)

        W = np.linalg.inv(K + self.reg * np.identity(K.shape[0]))

        XY = K.T @ W @ L
        return pairwise_distances(XY, metric="l2", n_jobs=self.workers)
    ...
    def test(self, X, Y, z, reps=1000, workers=1, random_state=None):
        r"""
        ...
        *args : ndarray
        ...
        e_hat = LogisticRegression(
            n_jobs=self.workers, penalty='l2', warm_start=True,
            solver='lbfgs', C=1 / self.reg).fit(
                K, z
        ).predict_proba(K)[:, 1]

        # Cannot parallelize due to storage cost of kernel matrices?
        perm_stats = np.zeros(reps)
        for k in range(reps):
            z_tilde = np.random.binomial(1, e_hat)
            perm_stats[k] = self._statistic(K, L, z_tilde)
        pvalue = (1 + np.sum(perm_stats >= stat)) / (1 + reps)
